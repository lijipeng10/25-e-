...
| 项目 | 值 | 怎么量的 | 日期 |
| --- | --- | --- | --- |
| **电机启动死区** | **≤ 10** | KEY2 升档法（见下），10 档时两轮已能持续稳定转动 | 2026/09/14 |
| **传感器 8 路左右顺序** | **正确** | 黑物从车左侧划到右侧，`S=` 里的 `1` 从最左一位走到最右一位，`E` 从 -100 到 +100 | 2026/09/14 |
| **两轮对称性（KEY2 直行）** | 轻微往**左**偏 | 两轮发完全相同的指令时车往左拐 → 右轮略弱 → `LF_TRIM` 用 **+2** | 2026/09/14 |

**KEY2 升档法**（一次烧录就能量出来）：

```
KEY2 每按一次升一档占空比:  0 -> 10 -> 12 -> 14 -> 16 -> 18 -> 20 -> 0 ...
车拿在手上(轮子离地)，一直按，看哪一档两个轮子开始能【持续稳定转动】
串口会打 [KEY2] motor test duty NN%，屏幕 L:/R: 也实时显示
```

> ⚠️ 注意区别：**「抖一下就不动」不算**，要的是持续稳定地转。

**这个数据有什么用**：`LF_BASE_DUTY` 要留出余量，保证循迹时差速把某一侧
减掉几个点之后**不会掉到死区以下**（掉下去那一侧就停转，车会一顿一顿）。
按误差最小跳变 14 推算，实际用到的 `steer` 大致是 0/5/8/11/14，于是：

| `LF_BASE_DUTY` | 慢轮（按误差 0/28/42/57/71） | 结论 |
| --- | --- | --- |
| **18**（现在） | 18 / 13 / 10 / 7 / 4 | 到 3 路误差才踩死区 ✅ |
| 14 | 14 / 9 / 6 / 3 / 0 | 2 路误差就踩死区，容易一顿一顿 ❌ |

> 📌 **一条被推翻的旧判断**：曾经怀疑「左右摆是因为基础速度贴着死区」，
> 实测死区只有 ≤10 而当时基础速度是 14，差得很远 —— **那个判断是错的**。
