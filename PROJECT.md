...
- 编码器计数是**双边沿**（A 相上升 + 下降都进中断）→ **每圈 520 脉冲**（`ENCODER_PULSE`）。
  单边沿是 260。★ 这两个数必须同步改：`empty.syscfg` 里 E1A/E2A 的极性
  `RISE_FALL` ↔ `RISE`，对应 `encoder.h` 的 `ENCODER_PULSE` 520 ↔ 260。
  验证分辨率：手慢慢转轮子，屏幕 `L`/`R` 能显示的最小非零值应该是 **±0006**
  （单边沿时是 ±0012）—— 因为 1 个脉冲/50ms = 5.8 mm/s。
