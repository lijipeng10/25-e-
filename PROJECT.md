...
- [ ] `LF_KP` / `LF_KD` —— 画龙(KP 大/KD 小)与拐不过来(KP 小)的折中
- [ ] `LF_BASE_DUTY` —— 基础速度, 现在 40%(电机死区约 30%, 别往下调太多)
- [ ] `LF_LOST_TIMEOUT_MS` —— 丢线多久停车, 现在 500ms

### 7.2 串口调试开关

`empty.c` 顶部有 `#define DBG_UART 1`:

- `1` = 每 100ms 往 UART2(PB15, 115200)打印一行 `S=... AD=... E=... L=... R=...`
- `0` = 整段打印代码不参与编译, 主循环不再被串口阻塞

**为什么需要这个开关**: 串口发送是死等的, 一行约 40 字符 ≈ 3.8ms,
会把 10ms 的循迹控制周期拖出抖动。调参时开着方便, 正式跑车建议关掉。
