...
里面的 `LF_PIVOT_MIN_DEG / MAX_DEG / OK` 是**从 `line_follow.c` 解析出来的**,
不会各写一份导致假通过; 还有一组"关掉角度门必须失败"的**阴性对照**证明测试有效。
...
| 双环二跑（第二次实车） | **"能转回来识别到走过来的线逆行"** —— 原地转个不停，转过半圈从背后重新看到**来路**那条线，停下后反着走回去；转弯仍然太快不稳定 | 赛道实测。根因是原地转向**只有计时没有角度**（旧的 1300ms 按 150 度/秒估，实际快一倍多，一遍能扫近 400 度）。已改用陀螺仪量角度：`LF_PIVOT_MIN_DEG` / `LF_PIVOT_MAX_DEG` |
...
LF_PIVOT_MIN_DEG    400    0.1度=40度。★ 转过它之前【不许】判"对准了" ——
                           防的是"刚起转就锁上来路那条线然后逆行"
LF_PIVOT_MAX_DEG   1100    0.1度=110度。★ 朝一边最多转这么多就掉头。
                           它同时是安全线：看到来路要转到约 158 度, 110+22 够不着
...
- [ ] **★★ 原地转向改成了"按角度转"，但【还没上赛道验证】** —— 这是当前最优先。
      要确认的现象：过弯时**扫到新线就停**、**不再原地转个不停**、
      **不再沿来路逆行**、车头转角不再超过 110 度。
      逻辑层面已用 `node test/pivot_sim.mjs` 仿真过（8/8 通过 + 阴性对照有效），
      但**仿真用的简化几何**，真实角度是否够用只能上赛道量
- [ ] **★ 转弯仍然"太快不稳定"（第二次实车反馈）** —— 原地转向那一块现在被角度
      卡住了，但【行进中的动态转弯】还是靠内环全速去追 `psi_ref`。
...
| 原地转向仿真测试 | 宿主仿真（Node） | `test/pivot_sim.mjs` | `node test/pivot_sim.mjs`；参数从 C 源里解析；**改状态机必须同步改它**，否则假通过 | Implemented |
