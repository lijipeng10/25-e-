...
LF_POS_KP            3     ★ 外环：位置误差 -> 目标航向（0.1 度 / 每格误差）
...
- [ ] **★ 双环(航向环)已实现，但【还没实车验证】** —— 代码 + 编译都过了
      （commit `0f5b712`），状态是 `Build Passed`。烧录后要确认的现象：
      参数页显示 `POS 3 / HED 5`、状态页 `H:` 随车头变化且**左转为正**、
      **拐角不再转反**、直道 `H` 的抖动明显小于 `R:`、过弯后 `H` 回到 0 附近
