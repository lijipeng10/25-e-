...
- **速度带符号**（正 = 前进，负 = 后退）：在 A 相跳变时**读 B 相的电平**（正交解码）——
  B 高算一个方向、B 低算另一个。★ **B 相不需要开中断**，只要配成输入即可，
  中断只挂在 A 相上。B 相线没接的话表现为「不管正转反转都显示负」，一眼能看出来。
  **符号（手转轮子实测，勿凭猜改）**：`ENCODER_2_SIGN = +1`（物理左轮 = B路 = E2A），
  `ENCODER_1_SIGN = -1`（物理右轮 = A路 = E1A，正转天然显示负，所以翻过来）。
  两轮符号不同是正常的 —— A/B 谁先跳变取决于接线和电机安装方向。
- 数的是 A 相**单边沿**（`empty.syscfg` 里 E1A/E2A 极性 = `FALL`）→ **每圈 260 脉冲**
  （用户实测确认）。★ 极性改成 `RISE_FALL`（双边沿）就变 520，`ENCODER_PULSE` 必须同步改；
  但**双边沿测不出方向**，两者是两件事。
- ★★ **物理左轮接在 B路（通道 2），物理右轮接在 A路（通道 1）** —— 宏在
  `line_follow.h` 的 `LF_LEFT_ID` / `LF_RIGHT_ID`。写错的后果是差速打到相反的轮子上：
  **线在左边车却往右拐，一起步就丢线**（实测踩过）。
  ⚠️ 这个错**看屏幕发现不了** —— 屏幕上的 `l`/`r` 是代码自己的通道命名，自洽。
  必须让真实轮子参与：轮子离地按住 KEY2，拿黑线在传感器底下划，线在右边时
  **物理左边那个轮子**应该转得更快。
