...
| 行 y | 内容 |
| --- | --- |
| 0 | `E` 循迹偏差(-100 线最左 ~ +100 线最右)、`P` 外环目标航向(度) |
| 12 | `Y` 陀螺仪实际航向(度)、`G` 8 路灰度位图(1 = 压线) |
| 24 | `L` / `R` 左右轮**实测速度**(mm/s) |
| 36 | `Emn` / `Emx` 本次运行期间的偏差摆幅 |
| 48 | 状态: `STOP  K2=GO` / `RUN` / `PIVOT` / `ERR-ENCODER` / `MPU:NO` |
...
**已知限制**：编码器只数单边沿、拿不到方向, `speed_1`/`speed_2` 恒为非负 ——
所以 `lf_pivot()` 里「一个轮子倒转」那一路实际不会倒转(速度环把负目标当 0),
表现为丢线后原地转不起来。要修得用 E1B/E2B 做正交解码。
