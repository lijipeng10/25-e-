...
- [x] **基础速度必须离开死区**: `LF_BASE_DUTY` 现在 18。
      ★ 反直觉的坑: 把基础速度从 14 往下调想"跑慢点", **左右摇摆反而更严重**。
      原因: 基础速度贴着死区时, 一个很小的转向命令(如 `steer=±2`)就会把某一侧
      推到死区**以下** -> 那个轮子直接不转 -> 车猛地往一边窜 -> 误差反号 ->
      另一侧又不转 -> 来回摆。**速度越低越严重。**
      所以差速摆动(±几)不能让任何一侧掉到死区以下, 要留余量。
- [x] **弯道: 停车原地转向再前进** (`LF_PIVOT_*` 四个参数)
      思路: 车重又慢, 靠差速硬拐急弯容易冲出去; 干脆到弯节点停住、
      原地把车头拧正、再前进。
      状态机(在 `line_follow_step` 里, 只有两个状态):
          LF:RUN    正常循迹
          LF:PIVOT  丢线 >= 80ms -> 判定到弯节点 -> 停住 -> 原地转向
                    -> 传感器重新扫到线且 |error| <= 20 -> 转回循迹
      判断"到弯了"靠丢线, 判断"转正了"靠线重新出现在中间,
      **不需要陀螺仪或编码器**。
      底层前提: `lf_set_wheel()` 现在支持**反转**(`cmd < 0`), 原地转向靠的
      就是左右轮一正一反。⚠️ 注意这也意味着循迹的差速输出必须先自己夹到 0 以上,
      否则大偏差时慢的那一侧会突然倒转、车会甩出去。
- [x] **直线左右摆: 加误差死区 `LF_DEADBAND = 15`**
      摇摆的真正来源是**数字量传感器的量化台阶**, 不是电机死区:
      误差最小跳变是 14, 车**停不住**在 ±14 上, 一动就跳到另一侧,
      于是 `steer` 在 `+2 / 0 / -2` 之间来回跳。
      死区把 |误差| = 14 这一档(只差一路)直接压成 0, 只在线偏到 2 路以上才修。
      代价: 车会在线中间 ±1 路以内自由浮动。
- [x] **弯道转向方向: 从"瞬时值"改成"方向证据"**
      原来用 `s_last_dir` 定方向, 但它是**瞬时值** —— 直线上误差在 ±14 之间抖,
      每次小修正都把它在 +1/−1 之间翻, 丢线那一刻记下的只是**最后一次无意义的
      抖动**, 所以"算不出往哪边转"。
      改成泄漏积分 `s_dir_ev = s_dir_ev*7/8 + error`, 只在看到线时更新:
      直线上的正负抖动互相抵消(证据贴 0), 弯道口横扫过来的横向线会连续几拍
      同号, 证据稳稳积累到一边。判断的是趋势, 不是噪声。
- [x] **弯道兜底: 一个方向找不到就掉头找另一边** (`LF_PIVOT_TRY_MS`)
      不管方向证据对不对, 两个方向都扫一遍总能扫到线, 不会原地转圈。
      两边都扫遍还是没有 -> 判定掉出赛道, 停车。
- [ ] `LF_PIVOT_TRIGGER_MS` / `LF_PIVOT_DUTY` / `LF_PIVOT_OK` / `LF_PIVOT_TRY_MS` 需实测调
