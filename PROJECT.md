...
- [ ] **★ 双环(航向环)已首跑，但还没调好** —— 第一次实车结果：
      直道**有改善**（不再明显摆尾，但仍有一点摆）；**转弯时车身转速太快**，
      线还没扫到就转过去了 -> 丢线。对应改动见下一版（`LF_POS_KP` 3->2、
      `LF_PIVOT_OK` 20->43），**还没再上赛道验证**
...
| 循迹 | **双环(航向环)**：外环 位置误差→目标航向，内环 航向差→转向量，再加陀螺阻尼；弯道停车原地转向 | `system/line_follow.c` | 参数全在文件顶部；有编译期检查兜错；★ **起步前必须把车摆正**（外环控制的是相对航向，见 §7.1） | Build Passed（待实车验证 + 赛道标定） |
