 ...
 *   P 项: err = 100 mm/s -> 5*100/100 = 5 个占空比单位; 约 err=400 时饱和。
 *   ★ 调法: 跟不上/超调 -> 动 MS_KP; 稳态总是差一截 -> 动 MS_KI。
 *
 * ★★ I 项的量程必须够到【满占空比】, 否则闭环等于没用 ★★
 *       I 项贡献 = ki * 积分 / 10000, 积分上限 = MS_I_CLAMP
 *       取 4 * 60000 / 10000 = 24 个占空比单位 -> 能覆盖 0~20 整个量程 ✓
 *   ★ 曾经取 MS_KI=1: 那样 I 最多只能给到 6 个占空比, 而实测【死区是 <=10】——
 *     也就是说 PI 根本顶不过死区, 低速档车压根不动。这个坑必须记着:
 *     I 项的量程是"能不能顶过死区"的关键, 不是随便给个数。 */
#define MS_KP               5
#define MS_KI               4
