...
void key_init(void)
{
    ...
    /* 按键扫描时基: main_timer, 用 SysConfig 配的装载值(50ms), 配成周期模式并启动。
       生成的代码是"一次性 + 不启动", 不重新配的话计数器起来就停, 中断进不去。 */
    cfg.timerMode  = DL_TIMER_TIMER_MODE_PERIODIC;
    cfg.period     = main_timer_INST_LOAD_VALUE;
    cfg.startTimer = DL_TIMER_START;
    DL_Timer_initTimerMode(main_timer_INST, &cfg);

    /* 开"计数到 0"中断, 中断里调 key_tick() 扫键 */
    DL_Timer_clearInterruptStatus(main_timer_INST, DL_TIMER_INTERRUPT_ZERO_EVENT);
    DL_Timer_enableInterrupt(main_timer_INST, DL_TIMER_INTERRUPT_ZERO_EVENT);

    NVIC_ClearPendingIRQ(main_timer_INST_INT_IRQN);
    NVIC_EnableIRQ(main_timer_INST_INT_IRQN);
}
