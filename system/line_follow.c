...
/* ---------- 转向极性(最关键的开关) ---------- */
/* +1 或 -1。判断方法: 让车跑起来, 如果车"朝着偏离方向越走越远"
 * (线在左边它还往左拐), 就把这个改成 -1。 */
#define LF_STEER_SIGN       (+1)

/* ---------- PID 参数 ---------- */
/* steer = (LF_KP * error + LF_KD * (error变化量)) / 100
 * error 范围 ±100, 所以:
 *     LF_KP = 40  -> 偏差满偏时 steer = 40 (正好等于基础速度, 一侧停另一侧全速)
 * 先只调 LF_KP, 觉得"过冲/画龙"再加一点 LF_KD。 */
#define LF_KP               40
#define LF_KI               0       /* 最基础版本先不用积分项 */
#define LF_KD               12

#define LF_MAX_STEER        40      /* 转向量上限。= LF_BASE_DUTY 时,
                                       极限情况下慢的一侧刚好停住, 不会反转 */
