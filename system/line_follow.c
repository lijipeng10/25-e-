 ...
 *   取 3 时的对照表:
 *       误差 ±14  -> 目标航向 ±4.2  度  (只偏一路, 轻微回正)
 *       误差 ±28  -> 目标航向 ±8.4  度
 *       误差 ±42  -> 目标航向 ±12.6 度
 *       误差 ±71  -> 目标航向 ±21.3 度  (急弯, 已经要大角度回正)
 *       误差 ±100 -> 目标航向 ±30   度  (拉满)
 *
 ...
 *   现在瞬时上约等于 外环3 * 内环5 / 100 = 0.15*error —— 同一个量级,
 *   所以换过来起步不会突然变猛。真正的差别在【动态过程】, 见上面的说明。
 *
 * 【LF_PSI_MAX】目标航向限幅(0.1 度)。LF_POS_KP=3 时 30 度就到顶了,
 *   根本用不到它; 它是给"把 LF_POS_KP 调得很大"兜底的 ——
 *   否则 psi_ref 会变成一个荒唐的角度(比如 300 度), 让内环一直顶着饱和。 */
#define LF_POS_KP           3       /* 外环: 位置误差 -> 目标航向 */
