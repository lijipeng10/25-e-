 ...
 *
 *   换成角度之后就与速度无关了, 而且能回答两个原来回答不了的问题:
 *       1) 已经转过 LF_PIVOT_MIN_DEG 了吗? 没过就不许停 ——
 *          因为来路的线就在起步那几十度里, 一停就锁错线 -> 逆行。
 *       2) 已经转过 LF_PIVOT_MAX_DEG 了吗? 过了就说明这个方向不对,
 *          掉头再找 —— 而不是继续转下去绕到来路那边。 */
#define LF_PIVOT_MIN_DEG    400     /* 0.1度, 即 40 度。
                                       ★ 朝当前方向转过它之前, 【不许】判"对准了"。
                                       H 题梯形赛道要转 76 度(C/D 角)或 105 度(A/B 角),
                                       而传感器阵列张角约 ±22 度 —— 所以新线大致要到
                                       转过 76-22 = 54 度时才进入"中间附近"。
                                       取 40 度: 既拦得住起步那几十度里的来路,
                                       又给新线留了 14 度余量。
                                       ★ 换赛道或换传感器宽度时, 这个数要跟着改。 */
...
#endif
#if (LF_PIVOT_MIN_DEG >= LF_PIVOT_MAX_DEG)
#error "LF_PIVOT_MIN_DEG 必须小于 LF_PIVOT_MAX_DEG: 否则可接受区间是空的, 原地转向永远对不准"
#endif
...
void line_follow_step(void)
{
    ...
    if (s_pivoting != 0U)
    {
        ...
        /* 判"对准了": 线在【中间附近】 ★而且★ 已经转过了最小角度。
         *
         * ★ 后面这半句是这次修的关键, 少不得:
         *   原地转向刚起步的那几十度里, 传感器扫到的多半还是【来路】那条线。
         *   没有角度门槛的话, 车会在那里就停下、然后沿着来路反着走 ——
         *   正是实车看到的"转回来识别到走过来的线逆行"。 */
        if ((on_line != 0U) && (error <= LF_PIVOT_OK) && (error >= -LF_PIVOT_OK)
            && (turned >= (int32_t)LF_PIVOT_MIN_DEG)) {
