 ...
 *
 *      steer = 0.15 * error  +  0.1 * LF_KD * (error 的变化量)
 *              └── 比例项 P ─┘    └──────── 微分项 D ────────┘
 *   (系数随 LF_KP 变: 比例项系数 = LF_KP/100, 所以 LF_KP=15 时是 0.15。
 *    之前这里写的 0.40 是 LF_KP=40 时代留下的, 早就对不上了, 已更正)
 *
 * 【为什么原来的 LF_KD = 12 不能要】
 ...
 *      0.1 * LF_KD * 14 <= 2.1 / 2   ->   1.4 * LF_KD <= 1.05   ->   LF_KD <= 0.75
 * 也就是说 LF_KP=15 这一档上, D 项【根本不该开】, 只能取 0。
 ...
 *   参考: 修正项 = LF_KP*error/100, LF_KP=15 时误差【拉满】(100)也只有 15。
 *
 * 【★ 符号: 已实测确认, 不要凭猜改 ★】
