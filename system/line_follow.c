...
/* ★★★ 转向增益和速度的关系 —— 这里连踩两个坑, 两种效应都写清楚, 别再想当然:
 *
 *    (1) 跟踪能力: 同样一个弯, 速度翻倍就需要加倍的角速度(w = v/R),
 *        所以增益越大弯道跟得越紧 —— 看起来应该"提速就加 KP";
 *    (2) 稳定性: 整个回路是【双积分】(转向 -> 角速度 -> 航向 -> 横向位置),
 *        环路增益 ∝ KP x 速度。速度翻倍而 KP 不动, 环路增益就翻倍 -> 振荡发散。
 *        ★ 实测: 440mm/s 配 KP=6(环路增益 4 倍), 车疯狂甩头, 一起步就丢线。
 *
 *    这两条是【矛盾】的: 速度越高, 能用的增益窗口越窄。
 *    这就是"纯比例控制跑不快"的根本原因。
 *
 *    ★ 所以提速时 KP 要【降】不要升。实测参考点(很稳, |E| 最大只有 14):
 *        220mm/s + KP 3 + KD 2
 *      按"环路增益 KP x 速度 不变"折算, 440mm/s 大致对应 KP 1.5~2。
 *    ★ 真想跑更快, 出路是【减小内侧速度环的延迟】(见 motor.c 的 motor_KP),
 *      不是加转向增益 —— 加增益只会把它推过稳定边界。 */
#define LF_STEER_KP     2       /* 每 1 格误差给多少差速 mm/s; error 最大 ±100 */
...
#define LF_GYRO_KD      4       /* ★★ 陀螺仪阻尼: 单位 mm/s 每 (度/秒)。0 = 关掉,
                                 ...
                                 *    ★ 阻尼越大, 系统振荡的自然频率越低、越稳
                                 *      (w_n ≈ sqrt(KP x 速度 / (轮距 + KD))),
                                 *      但它也会让转向变迟钝 —— 所以是"稳住之后尽量小" */
