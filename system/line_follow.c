...
/* ★★ 转向增益必须【跟着基础速度走】—— 这是实测踩过的坑, 别再只听速度改 BASE:
 *    差速决定的是角速度(w = 差速/轮距), 而【同样一个弯, 速度翻倍就需要加倍的角速度】
 *    (w = v/R, R 不变)。所以速度翻倍而 KP 不动, 同样的误差得到的转弯半径也翻倍
 *    -> 修正量相对速度只剩一半 -> 误差越积越大 -> 丢线。
 *    ★ 推荐配比:  KP ≈ 基础速度 / 73,  阻尼 KD ≈ 基础速度 / 110
 *        220mm/s -> KP 3, KD 2        440mm/s -> KP 6, KD 4 */
#define LF_STEER_KP     6       /* 每 1 格误差给多少差速 mm/s; error 最大 ±100 */
