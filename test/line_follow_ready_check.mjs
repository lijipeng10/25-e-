...
ok('满误差时快轮明显快、慢轮降到很慢(不一定是 0)', () => {
    // ★ KP 降下来以后(见 line_follow.c 顶部的长注释), 满误差的差速不一定能把慢轮压到 0;
    //   真正需要慢轮停住的直角弯, 是靠 COAST+TURN 原地转解决的, 不靠这里。
    assert.equal(cmdL(100), baseOf(100) + steer(100));
    assert.ok(cmdR(100) < cmdL(100) / 3, '慢轮没降下来: ' + cmdR(100));
    assert.ok(cmdR(100) >= 0, '慢轮命令不该是负的');
});
